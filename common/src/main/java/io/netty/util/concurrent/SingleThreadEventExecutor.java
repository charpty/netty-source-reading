 ...
 *
 * SingleThreadEventExecutor是EventLoop绑定单个线程并进行调度的典型实现，它和JDK的Executors.newSingleThreadExecutor()类似
 * 通过单个线程来调度执行任务，某个EventLoop上的事件都有同一个SingleThreadEventExecutor执行
 * SingleThreadEventExecutor是由netty自行实现的线程模型，而不采用JDK默认实现这是因为JDK的普通实现在高并发场景有局限性
 * 其中影响较大的是默认实现无法避免大量线程上下文切换带来的性能损耗
 */
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    private static final AtomicReferenceFieldUpdater<SingleThreadEventExecutor, ThreadProperties> PROPERTIES_UPDATER = AtomicReferenceFieldUpdater
            .newUpdater(SingleThreadEventExecutor.class, ThreadProperties.class, "threadProperties");
     ...
     *
     * @param parent
     *         the {@link EventExecutorGroup} which is the parent of this instance and belongs to it
     * @param threadFactory
     *         the {@link ThreadFactory} which will be used for the used {@link Thread}
     * @param addTaskWakesUp
     *         {@code true} if and only if invocation of {@link #addTask(Runnable)} will wake up the
     *         executor thread
     */
     ...
     */
    protected SingleThreadEventExecutor(EventExecutorGroup parent, ThreadFactory threadFactory, boolean addTaskWakesUp, int maxPendingTasks,
     ...
     *         the {@link EventExecutorGroup} which is the parent of this instance and belongs to it
     * @param executor
     *         the {@link Executor} which will be used for executing
     * @param addTaskWakesUp
     *         {@code true} if and only if invocation of {@link #addTask(Runnable)} will wake up the
     *         executor thread
     */
    ...
    public void execute(Runnable task) {
        ...
        boolean inEventLoop = inEventLoop();
        addTask(task);
        // 执行到该处说明任务是被其他线程放入到该Executor的，如果此时inEventLoop为false则代表当前thread为空
        if (!inEventLoop) {
            // 将this.thread初始化为调用者线程
            startThread();
    ...
    private void doStartThread() {
        assert thread == null;
        // executor一般为ThreadPerTaskExecutor，为每一个task都新起一个线程执行
            ...
            public void run() {
                ...
                try {
                    ...
                    if (success && gracefulShutdownStartTime == 0) {
                        logger.error("Buggy " + EventExecutor.class.getSimpleName() + " implementation; " + SingleThreadEventExecutor.class.getSimpleName()
